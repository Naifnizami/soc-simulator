...
class SOAREngine:
    def __init__(self, config, logger, azure_log="logs/defender_logs.json", falcon_log="logs/falcon_logs.json"):
        """
    ...
    def process_azure_alert(self, alert):
        ...
        try:
            ...
            if port in self.config.BLOCKED_PORTS and severity == "High":
                print(f"{Fore.RED}[BLOCK] High Sev Alert on Port {port} for {alert['azure_resource_id']}{Style.RESET_ALL}")
                
                self.logger.warning(f"SOAR ACTION: Blocking port {port} on {alert['azure_resource_id']}")
                return True
    ...
    def process_falcon_event(self, event):
        ...
        try:
            ...
            for signature in self.config.SUSPICIOUS_ARGS:
                if signature in cmd:
                    print(f"{Fore.YELLOW}[EDR] Host Isolation Triggered: {event['ComputerName']} (Reason: {signature}){Style.RESET_ALL}")
                    
                    self.logger.warning(f"SOAR ACTION: Isolating host {event['ComputerName']} due to signature '{signature}'")
                    return True
